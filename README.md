...
- Still *takelage* is highly customisable, transparent, free and open source software. Build your own takelship and ship any software setup with one command!
...
We recommend to install ruby as a user by using the Ruby Version Manager ([rvm](https://rvm.io/)) if you want to keep your host system clean.
...
The [`fortea-add-ssh-key`](ansible/roles/takel_ship_forgejo/templates/fortea-add-ssh-key.bash.j2) script serves as an example on how to query the fabulous forgejo API. Have a look at the [forgejo swagger](https://codeberg.org/api/swagger) page for all options:
